...
$query = "SELECT d.id, s.created_at FROM deal as d LEFT JOIN sydneytoday_newproduct_instance as s ON d.id=s.did WHERE d.valid=1 GROUP BY d.id ORDER BY s.created_at ASC LIMIT " . (($page-1) * $conf['record_each_page']) . ', ' . ($conf['record_each_page']);
...
$query = "SELECT d.id, s.created_at FROM deal as d LEFT JOIN sydneytoday_newproduct_instance as s ON d.id=s.did WHERE d.valid=1 GROUP BY d.id";
    ...
    <tbody>
      ...
      <tr id="deal-<?php echo $deal->getId(); ?>">
        ...
        <td>
          ...
          <!-- publish -->
          <button type="button" class="btn btn-xs btn-success publish-sydneytoday-newproduct" data-loading-text="Posting...">Post</button>
          <!-- validate -->
